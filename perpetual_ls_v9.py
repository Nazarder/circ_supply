...
perpetual_ls_v9.py
==================
Supply-Dilution L/S Strategy -- Version 9

v9 parameter changes vs v8 (de-overfitting):
  SUPPLY_WINDOW = 32 (was 26)   — grid-dominant; walk-forward consistently selects 32-40w
  LONG_QUALITY_LOOKBACK = 32 (tied to SW) — eliminates IS-specific LQ=12 peak
  BULL_BAND = 1.05 / BEAR_BAND = 0.95 (unchanged from v8)
  MOMENTUM_VETO_PCT = 0.0 (OFF) — 0.000 ablation dSharpe; dead weight
  LONG_QUALITY_VETO_PCT = 0.0 (OFF) — weakest veto; tied LQ=32w subsumes its function
  Net result: ~19 → ~15 free parameters; OOS SR > IS SR (genuine robustness)
  Expected: SR≈+0.966, Ann≈+17.11%, MaxDD≈-13.06%
...
SUPPLY_WINDOW        = 32               # v8: 26w fast window (was 13)
...
MOMENTUM_VETO_PCT    = 0.00   # v9: OFF (0.000 ablation dSharpe — dead weight)
...
LONG_QUALITY_VETO_PCT      = 0.00   # v9: OFF (tied SW=32 subsumes; IS-tuned lookback removed)
LONG_QUALITY_LOOKBACK      = 32     # v9: tied to SUPPLY_WINDOW (was 12)
