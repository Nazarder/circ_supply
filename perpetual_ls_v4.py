...
perpetual_ls_v4.py
==================
Supply-Dilution L/S Strategy -- Version 4

Uses ACTUAL Binance USDT-M perpetual futures data instead of synthetic estimates:

  [V4-A] Real prices: Binance weekly perp close prices replace CMC spot prices.
          This captures the perp premium/discount to spot and actual tradeable prices.
  [V4-B] Real funding rates: Binance 8h funding rates aggregated to weekly sums
          replace the regime-dependent synthetic funding model. Funding is now
          the actual amount paid/received per token, per period.
  [V4-C] Real ADTV: Binance USDT quote volume replaces CMC volume_24h for the
          inv-vol x sqrt(ADTV) position sizing.
  [V4-D] Real realised vol: computed from Binance weekly perp returns, not CMC spot.
  [V4-E] Universe restricted to tokens with active Binance USDT-M perp contracts.
          Onboard date used to prevent look-ahead bias (token only eligible after
          its perp listing date).
...
def run_backtest(df: pd.DataFrame, regime_df: pd.DataFrame,
    ...
    # BTC forward returns from Binance perp prices [V4-A]
    btc_fwd = {}
...
def print_report(res: dict) -> None:
    print("\n" + "=" * 76)
    print("PERPETUAL L/S BACKTEST v4  [ACTUAL BINANCE DATA]")
...
def plot_results(res: dict) -> None:
    ...
    fig.tight_layout()
    out1 = OUTPUT_DIR + "perp_ls_v4_cumulative.png"
    ...
    fig2.tight_layout()
    out2 = OUTPUT_DIR + "perp_ls_v4_regime_dd.png"
    fig2.savefig(out2, dpi=150); plt.close(fig2)
    print(f"[Plot] {out2}")

    # Figure 3: v3 vs v4 scorecard
    V3 = dict(
        combined_ann=-0.1376, combined_dd=-0.5979, sharpe=-0.448,
        win_rate=0.564, mean_spread=0.0374,
        bull_geo=0.8442, bear_geo=0.3734, side_geo=0.0215,
    )
    ...
    for regime in regimes_list:
        mask = regs == regime
        sub  = sp.iloc[list(np.where(mask)[0])]
        v4_mean[regime] = sub.mean() if len(sub) > 0 else np.nan
        v4_geo[regime]  = ((1 + sub.clip(lower=-0.99)).prod() ** (12/len(sub)) - 1
                           if len(sub) > 0 else np.nan)

    v4_st = portfolio_stats(res["combined_hedged"])

    fig3, axes3 = plt.subplots(2, 2, figsize=(14, 10))
    fig3.suptitle("v3 (synthetic costs) vs v4 (real Binance data) — Post-2022",
                  fontsize=13, fontweight="bold")
    x = np.arange(len(regimes_list)); w = 0.36

    ax6 = axes3[0, 0]
    ax6.bar(x - w/2, [V3["bull_geo"],  V3["bear_geo"],  V3["side_geo"]],
            w, label="v3 (synthetic)", color="silver", edgecolor="black")
    ax6.bar(x + w/2, [v4_geo.get(r, 0) for r in regimes_list],
            w, label="v4 (real data)", color="steelblue", edgecolor="black")
    ...
    ax7 = axes3[0, 1]
    # Funding comparison: synthetic vs actual
    fund_labels = ["Long drag\n(cum.)", "Short credit\n(cum.)", "Net\nimpact"]
    v3_funding  = [-0.1817, 0.3400, 0.1583]   # from v3 output
    v4_funding  = [res["fund_long_cum"], res["fund_short_cum"],
                   res["fund_long_cum"] + res["fund_short_cum"]]
    xf = np.arange(len(fund_labels))
    ax7.bar(xf - w/2, v3_funding, w, label="v3 (synthetic)", color="silver", edgecolor="black")
    ax7.bar(xf + w/2, v4_funding, w, label="v4 (actual)",    color="steelblue", edgecolor="black")
    ax7.axhline(0, color="black", lw=0.8)
    ax7.set_xticks(xf); ax7.set_xticklabels(fund_labels)
    ax7.set_title("Funding Attribution: Synthetic vs Actual")
    ax7.set_ylabel("Cumulative Funding"); ax7.legend(fontsize=9); ax7.grid(True, alpha=0.2)

    ax8 = axes3[1, 0]
    cum_v4h = (1 + res["combined_hedged"].dropna()).cumprod()
    cum_v4u = (1 + res["combined_net"].dropna()).cumprod()
    ax8.plot(cum_v4h.index, cum_v4h.values, "mediumseagreen", lw=2.5, label="v4 BTC-hedged (real)")
    ax8.plot(cum_v4u.index, cum_v4u.values, "steelblue",      lw=1.5, ls="--", label="v4 unhedged (real)")
    ax8.axhline(1, color="black", lw=0.6, ls="--")
    ax8.set_title("v4 Combined NAV"); ax8.set_ylabel("Cumulative Return")
    ax8.legend(fontsize=9); ax8.grid(True, alpha=0.2)

    ax9 = axes3[1, 1]
    ax9.axis("off")
    rows = [
        ["Metric",              "v3 (synthetic)",  "v4 (real data)"],
        ["Combined net (ann.)", f"{V3['combined_ann']:+.1%}",
         f"{v4_st['ann_return']:+.1%}"],
        ["MaxDD (BTC-hedged)",  f"{V3['combined_dd']:+.1%}",
         f"{v4_st['max_dd']:+.1%}"],
        ["Sharpe (hedged)",     f"{V3['sharpe']:+.3f}",
         f"{v4_st['sharpe']:+.3f}"],
        ["Win rate",            f"{V3['win_rate']:.1%}",
         f"{(sp > 0).mean():.1%}"],
        ["Mean spread",         f"{V3['mean_spread']:+.2%}",
         f"{sp.mean():+.2%}"],
        ["Bull geo spread",     f"{V3['bull_geo']:+.1%}",
         f"{v4_geo.get('Bull', float('nan')):+.1%}"],
        ["Bear geo spread",     f"{V3['bear_geo']:+.1%}",
         f"{v4_geo.get('Bear', float('nan')):+.1%}"],
    ]
    ...
    ax9.set_title("v3 vs v4 Scorecard", fontweight="bold", pad=14)

    fig3.tight_layout()
    out3 = OUTPUT_DIR + "perp_ls_v4_vs_v3.png"
...
def main():
    print("=" * 76)
    print("Supply-Dilution L/S Strategy -- Version 4  [REAL BINANCE DATA]")
