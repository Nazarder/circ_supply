"""
perpetual_ls_v7_full.py
=======================
Supply-Dilution L/S Strategy -- Version 7 (Full History)

Based on perpetual_ls_v7.py with the following additions:

  [FULL-1] START_DATE = None — use all available monthly dates from CMC data.
           The first valid periods will naturally start when Binance perp tokens
           have enough supply history (~early 2020).

  [FULL-2] Basket composition logging — basket_log appended per traded period,
           tracking long/short sets and per-period opens/closes.

  [FULL-3] Trade count tracking — total opens/closes per leg accumulated and
           reported.

  [FULL-4] Extended report — Pre-2021 vs 2021+ basket analysis, trade count
           summary, and multi-window performance comparison table.

  [FULL-5] Output CSV — basket_log saved to v7_full_basket_log.csv.
...
# [FULL-1] Use all available history (None = no date cutoff)
START_DATE  = None
WINS_LOW    = 0.01
WINS_HIGH   = 0.99
...
def run_backtest(df: pd.DataFrame, regime_df: pd.DataFrame,
    ...
    # [FULL-1] START_DATE = None means use all available dates
    ...
    # [FULL-2] Basket composition log
    basket_log = []

    # [FULL-3] Trade count accumulators
    total_long_opens  = 0
    total_long_closes = 0
    total_short_opens  = 0
    ...
    for i, t0 in enumerate(sorted_rebals[:-1]):
        ...
        # [FULL-2] Capture previous baskets BEFORE updating state
        ...
        # [FULL-2] Log basket composition for this period
        basket_log.append({
            "date":          t0,
            "regime":        regime,
        ...
        # [FULL-3] Accumulate trade counts (using same pre-update snapshots)
...
def print_report(res: dict) -> None:
    print("\n" + "=" * 78)
    print("PERPETUAL L/S BACKTEST v7 (FULL HISTORY)")
    ...
    print("  Window : Full history (START_DATE=None)")
    print("=" * 78)
    ...
    # -----------------------------------------------------------------------
    cutoff_2021 = pd.Timestamp("2021-01-01")
    bl = res.get("basket_log", [])

    pre2021 = [e for e in bl if e["date"] < cutoff_2021]
    print(f"\n  --- Pre-2021 Basket Analysis ---")
    n_pre = len(pre2021)
    print(f"  Periods before 2021-01-01: {n_pre}")
    if n_pre > 0:
        avg_long_pre  = np.mean([len(e["long"])  for e in pre2021])
        avg_short_pre = np.mean([len(e["short"]) for e in pre2021])
        print(f"  Avg long basket: {avg_long_pre:.1f} | Avg short basket: {avg_short_pre:.1f}")
        all_long_tokens_pre  = sorted(set(t for e in pre2021 for t in e["long"]))
        all_short_tokens_pre = sorted(set(t for e in pre2021 for t in e["short"]))
        print(f"  Long basket tokens (all appearances): {all_long_tokens_pre}")
        print(f"  Short basket tokens (all appearances): {all_short_tokens_pre}")
    else:
        print("  (no pre-2021 periods with trades)")

    # -----------------------------------------------------------------------
    # [FULL-4] Section B — Trade count
    # -----------------------------------------------------------------------
    ...
    total_trades = tlo + tlc + tso + tsc
    avg_per_period = total_trades / max(len(bl), 1)

    print(f"\n  --- Trade Count ---")
    print(f"  Long leg  : {tlo} opens, {tlc} closes ({tlo + tlc} total trades)")
    print(f"  Short leg : {tso} opens, {tsc} closes ({tso + tsc} total trades)")
    print(f"  Total trades: {total_trades}")
    print(f"  Avg trades per period: {avg_per_period:.1f}")

    # -----------------------------------------------------------------------
    # [FULL-4] Section C — Post-2021 vs full comparison table
    # -----------------------------------------------------------------------
    dates_idx = pd.DatetimeIndex(res["dates"])
    cn = res["combined_net"]

    def _window_stats(start_ts, end_ts=None):
        mask = dates_idx >= start_ts
        if end_ts is not None:
            mask = mask & (dates_idx < end_ts)
        if mask.sum() < 2:
            return None
        return portfolio_stats(cn[mask])

    st_full   = _window_stats(dates_idx[0] if len(dates_idx) > 0 else pd.Timestamp("2020-01-01"))
    st_pre21  = _window_stats(dates_idx[0] if len(dates_idx) > 0 else pd.Timestamp("2020-01-01"),
                              cutoff_2021) if n_pre > 0 else None
    st_2021p  = _window_stats(cutoff_2021)
    st_2022p  = _window_stats(pd.Timestamp("2022-01-01"))
    ...
    def _fs(st, key):
        ...
        if np.isnan(v):
            return "   N/A"
        if key in ("ann_return", "vol", "max_dd"):
            return f"{v:+.2%}"
        return f"{v:+.3f}"

    print(f"\n  --- Performance by Window ---")
    print(f"  {'Metric':<18} {'All periods':>13} {'Pre-2021':>13} "
          f"{'2021+':>13} {'2022+':>13}")
    print(f"  {'-'*74}")
    for label, key in [("Ann.Ret",  "ann_return"),
                        ("Vol",      "vol"),
                        ("Sharpe",   "sharpe"),
                        ("MaxDD",    "max_dd")]:
        print(f"  {label:<18} {_fs(st_full, key):>13} {_fs(st_pre21, key):>13} "
              f"{_fs(st_2021p, key):>13} {_fs(st_2022p, key):>13}")

    # -----------------------------------------------------------------------
    # v4 vs v6 vs v7(2022+) vs v7(full) comparison table
    # -----------------------------------------------------------------------
    ...
    # v7 (2022+ baseline window)
    st7_2022 = st_2022p if st_2022p is not None else {}
    # v7 (full history)
    st7_full = st_full if st_full is not None else {}

    def _f(v, fmt="+.1%"):
        return f"{v:{fmt}}" if (v is not None and not np.isnan(v)) else "  N/A"

    # Regime-conditional geo spreads for 2022+ and full
    def _geo_by_regime(start_ts, end_ts=None):
        mask = dates_idx >= start_ts
        if end_ts is not None:
            mask = mask & (dates_idx < end_ts)
        sp_w = sp[mask]
        ...
        for reg in ["Bull", "Bear", "Sideways"]:
            m2 = regs_w == reg
            sub = sp_w.iloc[list(np.where(m2)[0])]
            if len(sub) > 0:
                out[reg] = (1 + sub.clip(lower=-0.99)).prod() ** (12/len(sub)) - 1
            else:
                out[reg] = float("nan")
        return out

    v7_geo_2022 = _geo_by_regime(pd.Timestamp("2022-01-01"))
    v7_geo_full = _geo_by_regime(dates_idx[0] if len(dates_idx) > 0 else pd.Timestamp("2020-01-01"))

    sp_2022 = sp[dates_idx >= pd.Timestamp("2022-01-01")]

    # Win rate for 2022+
    wr_2022 = (sp_2022 > 0).mean() if len(sp_2022) > 0 else float("nan")
    ms_2022 = sp_2022.mean() if len(sp_2022) > 0 else float("nan")
    wr_full = (sp > 0).mean()
    ms_full = sp.mean()

    print(f"\n  --- v4 / v6 / v7(2022+) / v7(full) Comparison ---")
    print(f"  {'Metric':<28} {'v4 (base)':>11} {'v6':>11} {'v7(2022+)':>11} {'v7(full)':>11}")
    print(f"  {'-'*66}")
    print(f"  {'Combined net (ann.)':<28} {_f(V4['combined_ann']):>11} "
          f"{_f(V6['combined_ann']):>11} "
          f"{_f(st7_2022.get('ann_return', float('nan'))):>11} "
          f"{_f(st7_full.get('ann_return', float('nan'))):>11}")
    print(f"  {'MaxDD':<28} {_f(V4['combined_dd']):>11} "
          f"{_f(V6['combined_dd']):>11} "
          f"{_f(st7_2022.get('max_dd', float('nan'))):>11} "
          f"{_f(st7_full.get('max_dd', float('nan'))):>11}")
    print(f"  {'Sharpe':<28} {V4['sharpe']:>+11.3f} {V6['sharpe']:>+11.3f} "
          f"{_f(st7_2022.get('sharpe', float('nan')), '+.3f'):>11} "
          f"{_f(st7_full.get('sharpe', float('nan')), '+.3f'):>11}")
    print(f"  {'Win rate (spread)':<28} {V4['win_rate']:>11.1%} {V6['win_rate']:>11.1%} "
          f"{wr_2022:>11.1%} {wr_full:>11.1%}")
    print(f"  {'Mean period spread':<28} {V4['mean_spread']:>+11.2%} "
          f"{V6['mean_spread']:>+11.2%} {ms_2022:>+11.2%} {ms_full:>+11.2%}")
    print(f"  {'Bear geo spread':<28} {_f(V4['bear_geo']):>11} "
          f"{_f(V6['bear_geo']):>11} "
          f"{_f(v7_geo_2022.get('Bear', float('nan'))):>11} "
          f"{_f(v7_geo_full.get('Bear', float('nan'))):>11}")
    print(f"  {'Bull geo spread':<28} {_f(V4['bull_geo']):>11} "
          f"{_f(V6['bull_geo']):>11} "
          f"{_f(v7_geo_2022.get('Bull', float('nan'))):>11} "
          f"{_f(v7_geo_full.get('Bull', float('nan'))):>11}")

    side_v7_2022 = ("(skip)" if np.isnan(v7_geo_2022.get("Sideways", float("nan")))
                    else _f(v7_geo_2022.get("Sideways", float("nan"))))
    side_v7_full = ("(skip)" if np.isnan(v7_geo_full.get("Sideways", float("nan")))
                    else _f(v7_geo_full.get("Sideways", float("nan"))))
    print(f"  {'Sideways geo spread':<28} {_f(V4['side_geo']):>11} "
          f"{_f(V6['side_geo']):>11} {side_v7_2022:>11} {side_v7_full:>11}")
    print("=" * 78)
...
def plot_results(res: dict) -> None:
    ...
    fig.suptitle(
        "Supply-Dilution L/S v7 (Full History) | Monthly rebal | Sideways=cash | No BTC hedge\n"
        "Momentum veto on shorts | 2-layer signal (13w+52w) | Symmetric L/S (0.75/0.75)",
    ...
    ax.set_title("Cumulative Wealth — v7 Full History")
    ...
    ax2.plot(cum_c.index, cum_c.values, "mediumseagreen", lw=2.5, label="Combined net (v7 full)")
    ...
    ax4.set_title("Per-Period Gross Spread by Regime (v7 Full History)")
    ...
    ax5.fill_between(dd7.index, dd7.values, 0, color="mediumseagreen", alpha=0.55,
                     label="v7 full combined net")
    ax5.set_ylabel("Drawdown"); ax5.set_xlabel("Date")
    ax5.set_title("Drawdown: v7 Full History Combined Net")
    ax5.legend(fontsize=9); ax5.grid(True, alpha=0.2)
    fig2.tight_layout()
    out2 = OUTPUT_DIR + "perp_ls_v7_full_regime_dd.png"
    fig2.savefig(out2, dpi=150); plt.close(fig2)
    print(f"[Plot] {out2}")

    # Figure 3: v6 vs v7 scorecard (updated for full history)
    V6 = dict(
    ...
    fig3.suptitle("v6 (regime-aware freq) vs v7 Full History (structural fixes)",
    ...
    ax6.bar(x + w/2, v7_geos_bar, w, label="v7 full", color="mediumseagreen", edgecolor="black")
    ...
    ax7.plot(cum_v7.index, cum_v7.values, "mediumseagreen", lw=2.5, label="v7 full combined net")
    ax7.axhline(1.0, color="black", lw=1.0, ls="--", label="Breakeven")
    ax7.set_title("v7 Full History Combined NAV"); ax7.set_ylabel("Cumulative Return")
    ax7.legend(fontsize=9); ax7.grid(True, alpha=0.2)

    ax8 = axes3[1, 0]
    dd = (cum_v7 - cum_v7.cummax()) / cum_v7.cummax()
    ax8.fill_between(dd.index, dd.values, 0, color="mediumseagreen", alpha=0.55, label="v7 full")
    ax8.set_ylabel("Drawdown"); ax8.set_xlabel("Date")
    ax8.set_title("Drawdown: v7 Full History Combined Net")
    ...
    ax9.set_title("v6 vs v7 Full History Scorecard", fontweight="bold", pad=14)

    fig3.tight_layout()
    out3 = OUTPUT_DIR + "perp_ls_v7_full_vs_v6.png"
    fig3.savefig(out3, dpi=150); plt.close(fig3)
    print(f"[Plot] {out3}")


# ===========================================================================
#  STEP 8 -- Save basket log CSV
# ===========================================================================

def save_basket_log(res: dict) -> None:
    """[FULL-5] Save basket_log to CSV."""
    bl = res.get("basket_log", [])
    if not bl:
        print("[CSV] No basket log entries to save.")
        return
    ...
    for entry in bl:
        rows.append({
            ...
            "regime":        entry["regime"],
            "long_tokens":   ",".join(entry["long"]),
            "short_tokens":  ",".join(entry["short"]),
            "long_opens":    ",".join(entry["long_opens"]),
            "long_closes":   ",".join(entry["long_closes"]),
            "short_opens":   ",".join(entry["short_opens"]),
            "short_closes":  ",".join(entry["short_closes"]),
        })

    out_csv = "D:/AI_Projects/circ_supply/v7_full_basket_log.csv"
    pd.DataFrame(rows).to_csv(out_csv, index=False)
    print(f"[CSV] Basket log saved: {out_csv}  ({len(rows)} rows)")
...
def main():
    print("=" * 78)
    print("Supply-Dilution L/S Strategy -- Version 7 (Full History)")
    print("Monthly rebal | Sideways=cash | No BTC hedge | Momentum veto on shorts")
    print("2-layer signal (13w+52w) | Symmetric L/S (0.75/0.75) | START_DATE=None")
    print("=" * 78)
